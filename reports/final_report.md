...
### 3.4 研究点 4：RP2+RP3 组合实验

**方法**：先对语料库文本执行 RP3 抽取式压缩（ratio=0.3），再对压缩后的文本执行 RP2 分块聚合（QMSum/2Wiki: chunk_size=1024, overlap=128；ArguAna: chunk_size=512, overlap=64），最后进行 mean-pooling 编码。组合流水线为"语义压缩 → 分块编码 → mean-pooling 聚合"。

> 脚本: QMSum/2WikiMultihop 使用 `scripts/run_rp23_final.py`（已扩展 combined 模式），max_corpus=100, max_queries=50；ArguAna 沿用 `scripts/run_rp23_fast.py`（已有 combined 模式），max_corpus=200, max_queries=50。
...
| RP2-Chunk-Mean | 0.3173 (+90.0%) | 0.2613 (+27.9%) | 0.8313 (-3.7%) |
| RP3-Extractive | 0.2513 (+50.5%) | 0.2143 (+4.9%) | 0.8150 (-5.6%) |
| **RP2+RP3 组合** | **0.2446 (+46.5%)** | **0.2327 (+13.9%)** | **0.8221 (-4.8%)** |

**核心发现**：RP2+RP3 组合的效果介于单独 RP2 和单独 RP3 之间，在三个数据集上均未超越单独的 RP2 分块聚合。

- **QMSum (+46.5%)**：组合后相比 RP2 单独（+90.0%）下降了 43.5 pp，相比 RP3 单独（+50.5%）下降了 4.0 pp。压缩操作将 100 篇长文本从 1,621 chunks 压缩至 147 chunks（减少 91.0%），大幅减少了编码开销，但也损失了 RP2 原本能利用的跨句上下文信息——压缩后的文本不再是完整文档，分块聚合无法恢复被丢弃句子的语义连接。
- **2WikiMultihop (+13.9%)**：组合后优于 RP3 单独（+4.9%），但不及 RP2 单独（+27.9%）。压缩后 chunk 数从 1,208 降至 235（减少 80.5%）。百科文本信息密度高，压缩保留的 Top-30% 句子仍覆盖了大部分关键事实，分块在此基础上恢复了部分位置多样性，因此组合效果介于两者之间。
- **ArguAna (−4.8%)**：三种方法在短文本上均衰退，组合也不例外。与 RP2 单独（−3.7%）和 RP3 单独（−5.6%）相比，组合衰退幅度居中，未出现叠加恶化（即未出现 −3.7% + −5.6% = −9.3%），说明压缩在一定程度上保留了论辩核心节点，但这两步操作在短文本上本就缺乏发挥空间。

**方法间关系**：RP2（分块聚合）通过增加位置多样性来保留语义；RP3（语义压缩）通过减少冗余来聚焦核心信息。两者组合时，压缩先"提纯"文本、分块再"展开"——但这种展开是在已被压缩的信息上进行的，语义多样性已不可逆地减少。因此组合的精度上限受限于 RP3 已丢弃的信息量，在冗余度越高（QMSum）或信息越密集（2Wiki）的场景下，这一 trade-off 越明显。

> **效率权衡**：QMSum 场景下，组合方法中分块编码步骤仅 97s（纯 RP2 为 961s，减少 89.9%），chunk 数量从 1,621 降至 147。但 RP3 句子编码步骤本身耗时 1,657s，因此总耗时上组合（≈1,754s）多于纯 RP2（≈1,026s）。组合方法的优势在于"若已决定做语义压缩（如需要摘要/预览等副产品），叠加分块聚合的边际成本很低（97s），且仍能获得 +46.5% 的检索精度提升"。
...
| RP2+RP3 组合 | +46.5% | +13.9% | -4.8% | 需要压缩副产品 + 分块恢复的场景 |

> **ArguAna 为何四种方法均衰退？** ArguAna 论辩文本极短（100~300 tokens），四种方法均以长文本为设计目标：加权池化（RP1）在短文本上 token 范数差异小、区分度不足；分块聚合（RP2）的 chunk_size=512 导致 ≈1 chunk，分块无效果反而损失跨句上下文；语义压缩（RP3）在 ≤3 句时跳过压缩、直接返回原文；组合方法（RP2+RP3）继承了两者在短文本上的根本限制，但未出现叠加恶化（−4.8% vs 单独 RP2 −3.7% / RP3 −5.6%）。四个实验的负结果共同验证了一个前提：**这些方法的前提是文本足够长**（≥1000 tokens）。
...
├── run_rp23_final.py                # RP2 + RP3 + RP2+RP3 组合（QMSum/2Wiki）
└── run_rp23_fast.py                 # RP2 + RP3 + RP2+RP3 组合（ArguAna）
